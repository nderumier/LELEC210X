...
FINAL_PRODUCTION_MODEL_PATH = os.path.join(MODEL_DIR, "model_mlp_V1.pth")
TARGET_SHAPE = (20, 20)
...
# How many frequency bands (rows) do you want to hide? (0 to 20)
NUM_BANDS_TO_HIDE = 0 
# Strategy for hiding:
# 'random' : Drops different random bands per file (Great for Data Augmentation/Robustness)
# 'top'    : Consistently drops the highest frequency bands (Simulates low-pass filter)
# 'bottom' : Consistently drops the lowest frequency bands (Simulates high-pass filter)
...
# --- HELPER FUNCTION: FREQUENCY MASKING ---
def hide_frequency_bands(matrix, num_bands, strategy='random'):
    if num_bands <= 0:
        return matrix
        
    masked_matrix = matrix.copy()
    max_bands = matrix.shape[0] # Assuming rows are frequency bins
    
    if strategy == 'random':
        bands_to_mask = random.sample(range(max_bands), num_bands)
    elif strategy == 'top':
        # Assuming higher indices = higher frequencies
        bands_to_mask = list(range(max_bands - num_bands, max_bands))
    elif strategy == 'bottom':
        # Assuming lower indices = lower frequencies
        bands_to_mask = list(range(num_bands))
    else:
        bands_to_mask = []
        
    # Zero out the selected frequency rows
...
for filename in all_files:
    ...
    if spec_matrix_orig.shape == TARGET_SHAPE:
        ...
        if len(background_matrices) > 0:
            ...
            # Apply the masking function to the augmented matrix
